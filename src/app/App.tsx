...
export default function App() {
        ...
        try {
          ...
          if (data === 'RECEIVED' || data === 'CONFIRMED') {
            ...
            await supabase.from('doacoes_confirmadas').insert([{ 
                nome_pessoa: "Doador PIX (Confirmado)", 
                item_id: selectedItem?.itemId,
  ...
  }, [showPixScreen, pixData, selectedItem, quantity, selectedItemData]);

  useEffect(() => {
    const carregarEstoqueGlobal = async () => {
      if (estoqueCarregado) return;
      const { data, error } = await supabase.from('doacoes_confirmadas').select('item_id, quantidade');
      if (data && !error) {
          ...
          prevCategories.map((category) => ({
            ...
            items: category.items.map((item) => {
              const reservasBanco = data
                .filter((d) => String(d.item_id).trim() === String(item.id).trim())
                      ...
                      return (
                        <Card key={item.id} onClick={() => !isComplete && handleReserve(catIdx, item.id)} className={`bg-white/[0.03] border-[#D4AF37]/15 hover:border-[#D4AF37]/40 transition-all duration-500 group ${isComplete ? 'opacity-50' : 'cursor-pointer hover:bg-white/[0.07] hover:-translate-y-2'}`}>
                          ...
                          <CardContent className="pt-3 text-left">
                            ...
                            {!isComplete && (
                              <Button className="w-full hero-cta text-black font-bold uppercase text-[0.75rem] tracking-[0.3em] h-14 rounded-sm shadow-xl hover:scale-[1.01]">Contribuir com este Item!
                                <Heart size={18} className="ml-3" fill="currentColor" /></Button>
                            )}
                          </CardContent>
                        </Card>
                      );
